...
def train(opt, device, train_loader, val_loader, model, optimizer):
    ...
    for epoch in range(opt.epochs):
        ...
        for _, (input, input_h, name) in enumerate(tqdm(train_loader,desc='Training',file=sys.stdout)):
            ...
            # loss_recon_list.append(15*l_r.item())
            # loss_maxhist_list.append(l_sc.item())
            # loss_tv_list.append(l_l.item())
            # loss_col_list.append(2*l_hue.item())
            # loss_light_list.append(2*l_exp.item())
            # A_list2.append(torch.mean(A).item())
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm(model.parameters(), opt.grad_clip_norm)
            optimizer.step()
        print('Train loss: ', sum(epoch_loss)/len(epoch_loss))
        # print('\tRecon loss: '.format(num=epoch), sum(loss_recon_list)/len(loss_recon_list))
        # print('\tMaxhist loss: '.format(num=epoch), sum(loss_maxhist_list)/len(loss_maxhist_list))
        # print('\tTV loss: '.format(num=epoch), sum(loss_tv_list)/len(loss_tv_list))
        # print('\tcolor loss: '.format(num=epoch), sum(loss_col_list)/len(loss_col_list))
        # print('\tlight loss: '.format(num=epoch), sum(loss_light_list) / len(loss_light_list))
        # print('\tA val_set: '.format(num=epoch),sum(A_list2)/ len(A_list2))
        ...
        if ((epoch + 1) % opt.val_period) == 0:
            ...
            with torch.no_grad():
                for _, (input, input_clahe, name) in enumerate(tqdm(val_loader,desc='Validating',file=sys.stdout)):
                    ...
                    # torchvision.utils.save_image(R, '/home/xhm/音乐/epoch{no}_R.jpg'.format(no=epoch))
                    # torchvision.utils.save_image(L, '/home/xhm/音乐//epoch{no}_L.jpg'.format(no=epoch))
                    # torchvision.utils.save_image(input_h_scr, '/home/xhm/音乐/epoch{no}_gama.jpg'.format(no=epoch))
...
def parse_opt(known=False):
    ...
    parser.add_argument('--save_graph', action='store_true', default=True, help='generate graph of training updating process')
